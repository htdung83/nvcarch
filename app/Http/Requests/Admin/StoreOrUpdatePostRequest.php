<?php

namespace App\Http\Requests\Admin;

use App\Traits\UtilsTrait;
use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Support\Str;

class StoreOrUpdatePostRequest extends FormRequest
{
    use UtilsTrait;

    /**
     * Determine if the user is authorized to make this request.
     */
    public function authorize(): bool
    {
        return auth()->check();
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array<string, \Illuminate\Contracts\Validation\ValidationRule|array<mixed>|string>
     */
    public function rules(): array
    {
        return [
            'title' => 'required|string|max:100',
            'slug' => 'required|string|max:255',
            'description' => 'required|string|max:255',
            'post_category_id' => 'required|integer',
            'content' => 'required',
            'enabled' => 'required|integer',
            'img_url' => 'nullable',
        ];
    }

    public function prepareForValidation(): void
    {
        $content = html_entity_decode($this->input('content'));
        $description = $this->input('description');

        if ($description == "") {
            $description = $content;
        }

        $this->merge([
            'slug' => Str::slug($this->input('title')),
            'content' => $this->removeRootDomainIn($this->input('content')),
            'post_category_id' => intval($this->input('category')),
            'enabled' => intval($this->input('enabled')),
            'description' => $this->extractNumberOfChar($description, 170),
            'img_url' => $this->removeRootDomainIn(
                $this->extractFirstImageSrcIn($content)
            )
        ]);
    }
}
